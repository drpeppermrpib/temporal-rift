...
| Ashen Warden companion sprites | **Articulation+barks on master, flag ON** (`WARDEN_SPRITE_ENABLED=true`; Continuity/source testing; r=13 / drawH=48) — ally **#3b**; commit **abe9e02** + docs pins **2114ce3** / **a55db1e**; **store ship deferred** with peer batch; **no APK / no version bump** | **No** | **No** | **No** |
| Ashen Scout companion sprites | **Drafted on master, flag OFF** (`SCOUT_SPRITE_ENABLED=false`; live procedural; flip → r=13 / drawH=38) — bright green + purple archer; soft-wire stub only (**no articulate/barks yet**); **no APK / no version bump / no live enable**; store deferred with peer batch | **No** | **No** | **No** |
...
| Backlog | **Ashen Warden** — **#3b articulation+barks on master, store deferred** | Sheets `4c66510`; articulation+barks **abe9e02** + docs pins **2114ce3** / **a55db1e** + `WARDEN_SPRITE_ENABLED=true` for Continuity/source testing; collision `r=13` / `drawH=48` locked; idle↔walk gait / attack on swipeT / death when downed / heavy plant squash/bob/lean; bark voice `warden` (deep metallic armored robot guardian — CONTACT/FIRE/HOLD LINE; not orc, not dog). Desktop `C:\Users\drpep\Desktop\TemporalRift-Warden-Sprites`. See `docs/warden-art-plan.md`. **No APK / no version bump / not store-shipped.** |
| Backlog | **Ashen Scout** — **art drafted, flag OFF; articulate later** | Sheets `assets/allies/scout/{idle,walk,attack,death}.png` + `_refs/`; bright green plates + purple aether; slim knight-robotics **bow/archer** (not Warden yellow tank; not Fallout/Destiny IP); `SCOUT_SPRITE_ENABLED=false`; collision `r=13` / `drawH=38` locked; soft-wire stub only — **no gait plant / barks yet**. Desktop `C:\Users\drpep\Desktop\TemporalRift-Scout-Sprites` (+ Pictures). See `docs/scout-art-plan.md`. **No APK / no version bump / no live enable.** Sentinel still pending. |

**Next batch suggested order** (user photos → art upgrades + buildings + ravager + stronghold later): (1) ~~ravager mesh~~ **art+articulation+barks coded (flag OFF)** (2a) ~~**Keep art**~~ **drafted (flag OFF)** (2b) ~~**camps**~~ **drafted (flags OFF)** (2c) ~~**Muster Hall**~~ **drafted (flag OFF)** (2d) ~~**Gold Vault**~~ **remeshed mine-mouth (flag OFF)** (2e) ~~**Aether Pit**~~ **drafted (flag OFF)** → **(3a) ~~Rover dog~~ articulation+barks ON master Continuity (store deferred)** → **(3b) ~~Warden~~ articulation+barks ON master Continuity (store deferred)** → **(3b rem) ~~Scout art~~ drafted flag OFF; articulate+barks next; Sentinel later** → (4) ~~fence brick stronghold systems~~ **coded on master (store deferred)** → (5) one update apply (town save + ravager/keep/camp/hall/vault/pit flags ON + rover+warden already ON Continuity + brickwall already ON + scout when ready).
...
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Warden — sheets + articulation+barks (**abe9e02** + docs **2114ce3** / **a55db1e**); `WARDEN_SPRITE_ENABLED=true` for Continuity testing (r=13 / drawH=48); store ship deferred. Ally #3b Warden done.
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Scout — sheets drafted; `SCOUT_SPRITE_ENABLED=false`; soft-wire stub only (**no articulate/barks yet**); collision `r=13` / `drawH=38`; bright green + purple bow archer. See `docs/scout-art-plan.md`. Sentinel still pending.
...
| [`scout-art-plan.md`](./scout-art-plan.md) | Ashen Scout / Rift Scout ally art (drafted flag OFF; articulate later; r=13 / drawH=38) |
