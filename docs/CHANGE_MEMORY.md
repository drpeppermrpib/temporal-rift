...
| Backlog | **Gold Vault** building art — **drafted, flag OFF** | Commit `080094b`; `assets/buildings/vault/{idle,damaged}.png`; `VAULT_SPRITE_ENABLED=false`; collision `r=40` unchanged; flag ON → `drawH=52` (stockier than hall 60). Desktop `TemporalRift-Vault-Sprites`. See `docs/vault-art-plan.md`. Aether Pit still pending; apply with stronghold batch |
