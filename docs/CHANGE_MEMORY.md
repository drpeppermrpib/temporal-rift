...
| Gold Vault building sprites (idle/damaged) | **Drafted on master, flag OFF** (`VAULT_SPRITE_ENABLED=false`; live procedural golddepot; flip → drawH=52, r=40 unchanged) — **ship deferred** with stronghold batch; Aether Pit still pending | **No** | **No** | **No** |
...
| Backlog | **Muster Hall** building art — **drafted, flag OFF** | `assets/buildings/hall/{idle,damaged}.png`; `HALL_SPRITE_ENABLED=false`; collision `r=46` unchanged; flag ON → `drawH=60` (between camp ~50 and Keep 72). Desktop `TemporalRift-Hall-Sprites`. See `docs/hall-art-plan.md`. Apply with stronghold batch |
| Backlog | **Gold Vault** building art — **drafted, flag OFF** | `assets/buildings/vault/{idle,damaged}.png`; `VAULT_SPRITE_ENABLED=false`; collision `r=40` unchanged; flag ON → `drawH=52` (stockier than hall 60). Desktop `TemporalRift-Vault-Sprites`. See `docs/vault-art-plan.md`. Aether Pit still pending; apply with stronghold batch |

**Next batch suggested order** (user photos → art upgrades + buildings + ravager + stronghold later): (1) ~~ravager mesh~~ **art+articulation+barks coded (flag OFF)** (2a) ~~**Keep art**~~ **drafted (flag OFF)** (2b) ~~**camps**~~ **drafted (flags OFF)** (2c) ~~**Muster Hall**~~ **drafted (flag OFF)** (2d) ~~**Gold Vault**~~ **drafted (flag OFF)** → **Aether Pit** optional → (3) ally/militia/laborer skins if photos (4) fence brick stronghold systems (5) one update apply (town save + ravager/keep/camp/hall/vault flags ON).
