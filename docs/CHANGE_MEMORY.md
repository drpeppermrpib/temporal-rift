...
| Ashen Rover / Rift Hound companion sprites | **Drafted on master (`cd8a042`), flag OFF** (`ROVER_SPRITE_ENABLED=false`; live procedural `drawRover`; flip → drawH=26, r=11 unchanged) — ally **#3a dog only**; Warden/Scout/Sentinel next; **no APK / no version bump / no live enable** | **No** | **No** | **No** |
...
| Backlog | **Ashen Rover / Rift Hound** companion art — **#3a drafted, flag OFF** (`cd8a042`) | `assets/allies/rover/{idle,walk,attack,death}.png` + `_refs/`; gunmetal cyber-dog + aether cyan (original mesh — not Boston Dynamics / branded IP); `ROVER_SPRITE_ENABLED=false`; collision `r=11` unchanged; flag ON → `drawH=26`. Desktop `C:\Users\drpep\Desktop\TemporalRift-Rover-Sprites`. No APK / no version bump / no live enable. See `docs/rover-art-plan.md`. **Next:** Warden / Scout / Sentinel (#3b) when user photos or says go |

**Next batch suggested order** (user photos → art upgrades + buildings + ravager + stronghold later): (1) ~~ravager mesh~~ **art+articulation+barks coded (flag OFF)** (2a) ~~**Keep art**~~ **drafted (flag OFF)** (2b) ~~**camps**~~ **drafted (flags OFF)** (2c) ~~**Muster Hall**~~ **drafted (flag OFF)** (2d) ~~**Gold Vault**~~ **remeshed mine-mouth (flag OFF)** (2e) ~~**Aether Pit**~~ **drafted (flag OFF)** → **(3a) ~~Rover dog~~ drafted (flag OFF)** → **(3b) Warden/Scout/Sentinel** when photos/go → (4) ~~fence brick stronghold systems~~ **coded on master (store deferred)** → (5) one update apply (town save + ravager/keep/camp/hall/vault/pit/rover flags ON + brickwall already ON).
...
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Rover / Rift Hound sheets drafted (`cd8a042`) — `ROVER_SPRITE_ENABLED=false`; live procedural; flip → drawH=26 / r=11. Ally #3a dog only; #3b next.
...
| [`rover-art-plan.md`](./rover-art-plan.md) | Ashen Rover / Rift Hound ally art (#3a drafted, flag OFF) |
