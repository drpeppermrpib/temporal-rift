...
| Timber/Supply Camp building sprites | **Drafted on master, flags OFF** (`CAMP_SPRITE_ENABLED`/`TIMBER_SPRITE_ENABLED`/`SUPPLY_SPRITE_ENABLED=false`; live procedural; flip → timber drawH=50 r=40 / supply drawH=46 r=36) — halls still pending; **ship deferred** with stronghold batch | **No** | **No** | **No** |
...
| Backlog | **Timber/Supply Camp** building art — **drafted, flags OFF; halls pending** | Commit `0484c67`; `assets/buildings/camp/{idle,damaged,timber-idle,timber-damaged,supply-idle,supply-damaged}.png`; `CAMP_SPRITE_ENABLED`/`TIMBER_SPRITE_ENABLED`/`SUPPLY_SPRITE_ENABLED=false`; flip → timber r=40 drawH=50 / supply(farm) r=36 drawH=46 (collision r unchanged). Desktop `TemporalRift-Camp-Sprites`. See `docs/camp-art-plan.md`. Muster/halls when user sends photos; apply with stronghold batch |

**Next batch suggested order** (user photos → art upgrades + buildings + ravager + stronghold later): (1) ~~ravager mesh~~ **art+articulation+barks coded (flag OFF)** (2a) ~~**Keep art**~~ **drafted (flag OFF)** (2b) ~~**camps**~~ **drafted (flags OFF)** → **halls/Muster** when photos (3) ally/militia/laborer skins if photos (4) fence brick stronghold systems (5) one update apply (town save + ravager/keep/camp flags ON).
