...
| Ashen Rover / Rift Hound companion sprites | **Articulation+barks on master, flag ON** (`ROVER_SPRITE_ENABLED=true`; Continuity/source testing; r=11 / drawH=26) — ally **#3a**; **store ship deferred** with peer batch; **no APK / no version bump** | **No** | **No** | **No** |
...
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Rover / Rift Hound — sheets + articulation+barks; `ROVER_SPRITE_ENABLED=true` for Continuity testing (r=11 / drawH=26); store ship deferred. Ally #3a; #3b next.
