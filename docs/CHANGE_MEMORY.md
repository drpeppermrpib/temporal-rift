...
| Rift Keep building sprites (idle/damaged) | **Drafted on master, flag OFF** (`KEEP_SPRITE_ENABLED=false`; live procedural keep; flip → drawH=72, r=54 unchanged) — **ship deferred** with stronghold batch | **No** | **No** | **No** |
| Timber/Supply Camp building sprites | **Drafted on master, flags OFF** (`CAMP_SPRITE_ENABLED`/`TIMBER_SPRITE_ENABLED`/`SUPPLY_SPRITE_ENABLED=false`; live procedural; flip → timber drawH=50 r=40 / supply drawH=46 r=36) — **ship deferred** with stronghold batch | **No** | **No** | **No** |
...
| Ashen Rover / Rift Hound companion sprites | **Articulation+barks on master, flag ON** (`ROVER_SPRITE_ENABLED=true`; Continuity/source testing; r=11 / drawH=26) — ally **#3a**; commit **`4b9b2ea`**; **store ship deferred** with peer batch; **no APK / no version bump** | **No** | **No** | **No** |
| Ashen Warden companion sprites | **Drafted on master, flag OFF** (`WARDEN_SPRITE_ENABLED=false`; live procedural figure; flip → drawH=48, r=13 unchanged) — ally **#3b** Warden art; weathered yellow knight-robotics; **no APK / no version bump / no articulation yet** | **No** | **No** | **No** |
...
| Backlog | **Rift Keep** building art — **drafted, flag OFF** | `assets/buildings/keep/{idle,damaged}.png`; `KEEP_SPRITE_ENABLED=false`; collision `r=54` unchanged; flag ON → `drawH=72`. See `docs/keep-art-plan.md`. Apply with stronghold batch |
| Backlog | **Timber/Supply Camp** building art — **drafted, flags OFF** | Commit `0484c67`; `assets/buildings/camp/{idle,damaged,timber-idle,timber-damaged,supply-idle,supply-damaged}.png`; `CAMP_SPRITE_ENABLED`/`TIMBER_SPRITE_ENABLED`/`SUPPLY_SPRITE_ENABLED=false`; flip → timber r=40 drawH=50 / supply(farm) r=36 drawH=46 (collision r unchanged). Desktop `TemporalRift-Camp-Sprites`. See `docs/camp-art-plan.md`. Apply with stronghold batch |
...
| Backlog | **Ashen Rover / Rift Hound** — **#3a articulation+barks on master, store deferred** | Sheets `cd8a042`; articulation+barks **`4b9b2ea`** + `ROVER_SPRITE_ENABLED=true` for Continuity/source testing; collision `r=11` / `drawH=26` locked; aether-hound bark voice + lines (distinct from orcs). Desktop `C:\Users\drpep\Desktop\TemporalRift-Rover-Sprites`. See `docs/rover-art-plan.md`. **No APK / no version bump / not store-shipped.** |
| Backlog | **Ashen Warden** — **#3b art drafted, flag OFF** | Sheets `assets/allies/warden/{idle,walk,attack,death}.png` + `_refs/` (knight + yellow mech mesh; not Warframe/Destiny IP); `WARDEN_SPRITE_ENABLED=false`; collision `r=13` / `drawH=48` locked; procedural cannon figure live. Desktop `C:\Users\drpep\Desktop\TemporalRift-Warden-Sprites`. See `docs/warden-art-plan.md`. **No APK / no version bump / no articulation yet.** **Next:** articulate + flag ON when user says go; Scout / Sentinel still pending |

**Next batch suggested order** (user photos → art upgrades + buildings + ravager + stronghold later): (1) ~~ravager mesh~~ **art+articulation+barks coded (flag OFF)** (2a) ~~**Keep art**~~ **drafted (flag OFF)** (2b) ~~**camps**~~ **drafted (flags OFF)** (2c) ~~**Muster Hall**~~ **drafted (flag OFF)** (2d) ~~**Gold Vault**~~ **remeshed mine-mouth (flag OFF)** (2e) ~~**Aether Pit**~~ **drafted (flag OFF)** → **(3a) ~~Rover dog~~ articulation+barks ON master Continuity (store deferred)** → **(3b) ~~Warden art~~ drafted flag OFF; Scout/Sentinel + Warden articulate when go** → (4) ~~fence brick stronghold systems~~ **coded on master (store deferred)** → (5) one update apply (town save + ravager/keep/camp/hall/vault/pit flags ON + rover already ON Continuity + brickwall already ON).
...
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Rover / Rift Hound — sheets + articulation+barks (`4b9b2ea`); `ROVER_SPRITE_ENABLED=true` for Continuity testing (r=11 / drawH=26); store ship deferred. Ally #3a.
- **Master later (unreleased; still 2.13.2 / vc46, no APK):** Ashen Warden — sheets drafted; `WARDEN_SPRITE_ENABLED=false` (r=13 / drawH=48); yellow knight-robotics mesh; articulate later. Ally #3b partial; Scout/Sentinel pending.
