...
| Ashen Rover / Rift Hound companion sprites | **Drafted on master, flag OFF** (`ROVER_SPRITE_ENABLED=false`; live procedural `drawRover`; flip → drawH=26, r=11 unchanged) — ally **#3a dog only**; Warden/Scout/Sentinel next; **no APK / no version bump / no live enable** | **No** | **No** | **No** |
...
| Backlog | **Ashen Rover / Rift Hound** companion art — **#3a drafted, flag OFF** | `assets/allies/rover/{idle,walk,attack,death}.png` + `_refs/`; gunmetal cyber-dog + aether cyan (original mesh — not Boston Dynamics / branded IP); `ROVER_SPRITE_ENABLED=false`; collision `r=11` unchanged; flag ON → `drawH=26`. Desktop `C:\Users\drpep\Desktop\TemporalRift-Rover-Sprites`. No APK / no version bump / no live enable. See `docs/rover-art-plan.md`. **Next:** Warden / Scout / Sentinel (#3b) when user photos or says go |
