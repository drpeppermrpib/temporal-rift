...
| Backlog | **Timber/Supply Camp** building art — **drafted, flags OFF; halls pending** | `assets/buildings/camp/{idle,damaged,timber-*,supply-*}.png`; `CAMP_SPRITE_ENABLED`/`TIMBER_SPRITE_ENABLED`/`SUPPLY_SPRITE_ENABLED=false`; timber r=40 drawH=50 / supply(farm) r=36 drawH=46. See `docs/camp-art-plan.md`. Muster/halls when user sends photos; apply with stronghold batch |
