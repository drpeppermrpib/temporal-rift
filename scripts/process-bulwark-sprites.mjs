/**
 * Process Ashen Bulwark sheets: black-bg flood-key → crop → pad → 512×512.
...
const SRC_DIR = join(ROOT, 'assets', 'bulwark');
...
async function processOne(name) {
  ...
  // Flood-fill transparent from corners so dark armor interior stays opaque
  ...
  // Soft edge: near-black neighbors of transparent get partial alpha
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (a === 255 && isNearBlack(i)) {
        // fringe soften if any transparent 4-neigh
        let touch = false;
...
console.log('bulwark sprites processed (no www sync — soft-wire / APK deferred)');
