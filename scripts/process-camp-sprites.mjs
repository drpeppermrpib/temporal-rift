/**
 * Process Timber/Supply Camp building sheets: light-bg flood-key → crop → pad → 512×512.
...
const SRC_DIR = join(ROOT, 'assets', 'buildings', 'camp');
const RAW_DIR = join(SRC_DIR, '_raw');
const SIZE = 512;
const LIGHT_MIN = 215; // near-white / soft studio grey
const CHROMA_MAX = 18; // low chroma = bg

const SHEETS = [
  'idle', 'damaged',
  'timber-idle', 'timber-damaged',
  'supply-idle', 'supply-damaged',
];
...
const www = join(ROOT, 'www', 'assets', 'buildings', 'camp');
mkdirSync(www, { recursive: true });
for (const name of SHEETS) {
  copyFileSync(join(SRC_DIR, `${name}.png`), join(www, `${name}.png`));
}
console.log('synced to www/assets/buildings/camp (no _refs/_raw)');
...
for (const name of SHEETS) {
  ...
  console.log(`${name} transparent%=${((z / t) * 100).toFixed(1)} cornerA=${data[3]}`);
}
