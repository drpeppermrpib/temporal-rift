/**
 * Process Muster Hall building sheets: light-bg flood-key → crop → pad → 512×512.
...
const SRC_DIR = join(ROOT, 'assets', 'buildings', 'hall');
...
const www = join(ROOT, 'www', 'assets', 'buildings', 'hall');
mkdirSync(www, { recursive: true });
for (const name of ['idle', 'damaged']) {
  copyFileSync(join(SRC_DIR, `${name}.png`), join(www, `${name}.png`));
}
console.log('synced to www/assets/buildings/hall (no _refs/_raw)');
