/**
 * Process Aether Pit building sheets: light-bg flood-key -> crop -> pad -> 512x512.
...
const SRC_DIR = join(ROOT, 'assets', 'buildings', 'pit');
...
const www = join(ROOT, 'www', 'assets', 'buildings', 'pit');
mkdirSync(www, { recursive: true });
for (const name of ['idle', 'damaged']) {
  copyFileSync(join(SRC_DIR, `${name}.png`), join(www, `${name}.png`));
}
console.log('synced to www/assets/buildings/pit (no _refs/_raw)');
