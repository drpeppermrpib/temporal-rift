/**
 * Process Ashen Shaman sheets: black-bg flood-key → crop → pad → 512×512.
 * Dark robes preserved (corner flood only — not global near-black key).
 */
...
const SRC_DIR = join(ROOT, 'assets', 'shaman');
...
async function processOne(name) {
  ...
  // Flood-fill transparent from corners so dark robe interior stays opaque
...
console.log('shaman sprites processed (no www sync — soft-wire / APK deferred)');
