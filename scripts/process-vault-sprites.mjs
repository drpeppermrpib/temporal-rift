/**
 * Process Gold Vault building sheets: light-bg flood-key → crop → pad → 512×512.
...
const SRC_DIR = join(ROOT, 'assets', 'buildings', 'vault');
...
const www = join(ROOT, 'www', 'assets', 'buildings', 'vault');
mkdirSync(www, { recursive: true });
for (const name of ['idle', 'damaged']) {
  copyFileSync(join(SRC_DIR, `${name}.png`), join(www, `${name}.png`));
}
console.log('synced to www/assets/buildings/vault (no _refs/_raw)');
