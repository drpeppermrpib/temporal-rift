/**
 * Process Ashen Warden sheets: light/grey-bg flood-key → crop → pad → 512×512.
 * Yellow ochre plates + cyan/violet aether accents preserved (corner flood only).
...
const SRC_DIR = join(ROOT, 'assets', 'allies', 'warden');
const RAW_DIR = join(SRC_DIR, '_raw');
const SIZE = 512;
const LIGHT_MIN = 160; // studio mid-grey (~166–191), not pure white
const CHROMA_MAX = 28;
const GREY_DIST = 48;
...
async function processOne(name, mode) {
  ...
  const isBg = (i) => {
    ...
    // preserve yellow ochre plates + cyan/violet aether
    const yellowish = r > g + 8 && r > b + 20 && g > b + 5 && mx > 90;
    const cyanish = b > r + 20 && b > g + 10;
    const violetish = (b > g + 15 && r > g + 10 && mx > 80) || (r > 80 && b > 100 && g < 90);
    if (yellowish || cyanish || violetish) return false;
    if (mode === 'light') {
      return mx >= LIGHT_MIN && (mx - mn) <= CHROMA_MAX;
    }
    // mid-grey studio flood from corner seed; keep dark mech charcoal (mx low)
...
const frames = [
  ['idle', 'warden-idle.png', 'grey'],
  ['walk', 'warden-walk.png', 'grey'],
  ['attack', 'warden-attack.png', 'grey'],
  ['death', 'warden-death.png', 'grey'],
];
...
const www = join(ROOT, 'www', 'assets', 'allies', 'warden');
mkdirSync(www, { recursive: true });
for (const [name] of frames) {
  copyFileSync(join(SRC_DIR, `${name}.png`), join(www, `${name}.png`));
}
console.log('synced to www/assets/allies/warden (no _refs/_raw)');
